...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-9999 flex h-screen w-65 flex-col overflow-y-hidden bg-gray-900 duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
    >
      <div className='flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5'>
        <NavLink to='/' className={`text-gray-200 font-bold ${sidebarOpen ? 'text-base' : 'text-2xl'}`}>
          SAVING BUNCH ADMIN
        </NavLink>
        {sidebarOpen && <div onClick={()=>{
          setSidebarOpen(!sidebarOpen);
        }}>
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi text-white bi-x-lg" viewBox="0 0 16 16">
